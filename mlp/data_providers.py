...
class DataProvider(object):
    ...
    def max_num_batches(self, value: int) -> None:
        if value == 0 or value < -1:
            raise ValueError("max_num_batches must be -1 or > 0")
        self._max_num_batches = value
        self._update_num_batches()

    def _update_num_batches(self) -> None:
    ...
    def __next__(self) -> Tuple[NDArray[np.floating], NDArray[np.floating]]:
        return self.next()
    ...
    def next(self) -> Tuple[NDArray[np.floating], NDArray[np.floating]]:
...
class MNISTDataProvider(DataProvider):
    ...
    def to_one_of_k(self, int_targets: NDArray[np.floating]) -> NDArray[np.floating]:
...
class EMNISTDataProvider(DataProvider):
    ...
    def label_smoothing(self, int_targets: NDArray[np.floating], alpha: float = 0.1) -> NDArray[np.floating]:
